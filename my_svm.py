...
x_user = int(input("Enter x: "))
y_user = int(input("Enter y: "))
width_user = int(input("Enter w: "))
height_user = int(input("Enter h: "))
w_user = width_user * height_user
x_.append(x_user)
y_.append(y_user)
w_.append(w_user)
x = le.fit_transform(x_)
...
def enterData(x_user, y_user, w_user):
      ...
      new_user = np.array([(x_user[-1], y_user[-1], w_user[-1])])
      new_out = model.predict(new_user)
      if new_out==0:
            print("Your result is: 'Left'")
      else:
